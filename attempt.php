...
$url = str_replace('/mod/quiz/', '/local/qrsub/', $attemptobj->attempt_url(null, $page));
$PAGE->set_url($url);
...
///////////////////////////////////////////////////////
// QRMOD-11 - POC a file sub in a Hybrid question type.
// Add body class to differenciate quiz from qrsub.
$PAGE->add_body_class('local_qrsub');
// QRMOD-11
...
//
// Add the AMD to manage the response summary default text.
$config = get_config('qtype_hybrid');
if (!empty($config->responsesummary_default_text)) {
    $default_msg = $config->responsesummary_default_text;
} else {
    $default_msg = get_string('responsesummary', 'local_qrsub');
}
$PAGE->requires->js_call_amd(
    'local_qrsub/response_summary_text',
    'init',
    array($default_msg)
);
// QRMOOD-41
...
// Encapsulate the business logic of the hybrid question nav block.
$qrsub_attempt_info_block = new qrsub_attempt_info_block($attemptobj);

// Build and render the nav panel.
$rendered_attempt_nav_panel = $qrsub_attempt_info_block->get_attempt_info_block();

// Add the panel to the page.
$regions = $PAGE->blocks->get_regions();
$PAGE->blocks->add_fake_block($rendered_attempt_nav_panel, reset($regions));
// QRMOD-11
...
///////////////////////////////////////////////////////
// QRMOD-11 - POC a file sub in a Hybrid question type.
if (qrsub::is_last_hybrid_question($attemptobj, $slots)) {
    $nextpage = -1;
} else {
    list($nextslots, $nextpage) = qrsub::get_next_hybrid_question($attemptobj, $page);
}
...
///////////////////////////////////////////////////////
// QRMOD-11 - POC a file sub in a Hybrid question type.
// Redirect here as long as attempt is not done.
$outputQrsub = $PAGE->get_renderer('local_qrsub');
$html = $outputQrsub->attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id, $nextpage);
$html = str_replace('/mod/quiz/processattempt.php', '/local/qrsub/processattempt.php', $html);
echo $html;
