...
if ($next && $nextpage !== -1) {
    // Go to the next hybrid question.
    list($slots, $page) = qrsub::get_next_hybrid_question($attemptobj, $thispage);

} else if ($next && $nextpage == -1) {
    // Going forward and ready to submit the quiz.
    $page = $nextpage;

} else if ($previous && $thispage > 0) {
    // Go to the previous hybrid question.
    list($slots, $page) = qrsub::get_previous_hybrid_question($attemptobj, $thispage);
...
if ($status == quiz_attempt::OVERDUE) {
    ///////////////////////////////////////////////////////
    // QRMOD-11 - POC a file sub in a Hybrid question type.
    $url = str_replace('mod/quiz', 'local/qrsub', $attemptobj->summary_url());
    redirect($url);
    ...
    ///////////////////////////////////////////////////////
    // QRMOD-11 - POC a file sub in a Hybrid question type.
    $url = str_replace('mod/quiz', 'local/qrsub', $nexturl);
    redirect($url);
    ...
    // QRMOD-11 - POC a file sub in a Hybrid question type.
    //
    // Exam is finished. Redirect to the Exam start page.
